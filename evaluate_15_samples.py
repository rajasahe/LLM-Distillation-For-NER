...
CSV_PATH = r"C:\Users\Raja\Coriolis\Final_Distillation\Test Data\test_ner_prompts_sampled_15_outputs_distilled_cleaned.csv"
...
def evaluate():
    ...
    results = {
        "zero_shot": {"Teacher": {"TP": 0, "FP": 0, "FN": 0}, "Distilled": {"TP": 0, "FP": 0, "FN": 0}},
        "one_shot":  {"Teacher": {"TP": 0, "FP": 0, "FN": 0}, "Distilled": {"TP": 0, "FP": 0, "FN": 0}},
        "two_shot":  {"Teacher": {"TP": 0, "FP": 0, "FN": 0}, "Distilled": {"TP": 0, "FP": 0, "FN": 0}},
    }
    ...
    print("="*80)
    print(f"{'Mode':<15} | {'Model':<12} | {'Precision':<10} | {'Recall':<10} | {'F1 Score':<10}")
    print("-" * 80)
    
    for mode in ["zero_shot", "one_shot", "two_shot"]:
        for model in ["Teacher", "Distilled"]:
            metrics = results[mode][model]
            p, r, f1 = compute_metrics(metrics["TP"], metrics["FP"], metrics["FN"])
            print(f"{mode:<15} | {model:<12} | {p:<10.4f} | {r:<10.4f} | {f1:<10.4f}")
        print("-" * 80)
